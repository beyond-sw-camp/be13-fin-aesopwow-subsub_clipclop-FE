...
export default function WatchTimePage() {
  const { filters, setFilters, users, isLoading, error } = useSegmentViewModel("watchTimeHour");
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      doc.setFontSize(14);
      doc.text("User Watch Time Data", 14, 15);
      ...
      autoTable(doc, {
        startY: 25,
        head: [selectedFields.map((title) => ({ content: title }))],
        body: rows,
        styles: {
          font: "NotoSansKR",
          fontSize: 10,
      ...
      const fileName = `watch_time_users_${new Date().toISOString().split('T')[0]}.pdf`;
      doc.save(fileName);
    } catch (error) {
